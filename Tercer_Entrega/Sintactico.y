...
int guardarTSIntFilter(int num);
...
struct node *stackCondicion = NULL, *stackOperador = NULL,*stackPosicion = NULL, * stackBiFilter=NULL,*stackAnd=NULL,*stackOr=NULL, *stackOpeFilter=NULL, *stackCondAssem=NULL,*stackWhile=NULL,*stackIf=NULL;
//prototipos
...
t_terceto crearTercetoFilter(char* operacion,char* t1,char* t2);
...
inicioFor:{auxForInicio=-1;}asignacion {auxForInicio=0;} ;

finFor:{auxForFinal=-1;} expresion {auxForFinal=0;};
...
concatenacion:
	factor {
		...
		factor_terceto_aux = factor_terceto;
	}
...
factor_sin_negar:
	...
	|CONST_BIN  { 
				 if (DEBUG) printf("factor_sin_negar->CONST_BIN\n");

				  printf("guardo %s\n",$<str_val>1); int pos = guardarTSBin($<str_val>1);
					
				flagEsString = NO_ES_CADENA;
				 char aux[20]="_";
				 strcat(aux,vectorSymbol[pos].valor);		
				
				 factor_terceto =  crearTerceto(aux,"-1","-1"); 
				 printf("guardo binario\n");
			
				}
	|CONST_HEXA { 
				  if (DEBUG) printf("factor_sin_negar->CONST_HEXA\n");

				  printf("guardo %s\n",$<str_val>1); int pos = guardarTSHexa($<str_val>1);
				  flagEsString = NO_ES_CADENA;
	...
	|longitud {
			...
			//GUARDAR EN LA TABLA DE SIMBOLO
			char buffer[11];
	itoa(contadorLong, buffer, 10);
	t_symbol aux= getSymbol(buffer, "CONST_INT", buffer,buffer,"");
	int pos=guardarEnVector(aux, 0);
...
listaID:
		...
		encolar($<str_val>1, &colaID);
			...
			if(filterActivado==1)
			{
			 ...
			 //******* validacion **************//
				...
				if(pos==-1) // no existe en la tabla
				{
					mostrarError(ERROR_ID_NO_DECLARADO,$<str_val>1);
				}
				else
					if(strcmp(vectorSymbol[pos].tipo,"integer"))
					{
						mostrarError(ERROR_TIPO_INCOMPATIBLE,$<str_val>1);
					}	
				...
				for(cont=0;cont<cantElemFilter;cont++)
				{
					sprintf(aux4,"%d",var.numeroTerceto);
					sprintf(aux5,"%s",vectorTercetosFilter[cont].t2);
					



					t_terceto condicion_if_terceto = crearTerceto(vectorTercetosFilter[cont].operacion,aux4,aux5);
					int op1;
					

					
					if(contOperLog==0) // cuando no hay operadores
						{
							push(STACK_OPERADOR, "0");
						}
						else	
						{	
							...
							if(cont>0) // si no es la primer pasada
							{

								if(strcmp(vecOpLog[cant], "AND")==0)  // SI ES UN AND T2 DEBE APUNTAR A DONDE TERMINA EL IF, SE DEBE APILAR
								{	
									
									sprintf(aux4,"%d",op1);

									sprintf(aux5,"%d",condicion_if_terceto.numeroTerceto);
									crearTerceto("and",aux4,aux5);
							...
							{
								// guardo el primer terceto de operador logico
								op1=condicion_if_terceto.numeroTerceto;

								sprintf(aux4,"%d",condicion_if_terceto.numeroTerceto);
								push(STACK_CONDICION,aux4);
							}
						}
				...
				////////////////////////////////
				// asigno filter:=var
				int valorIndic=indiceTerceto+1;
				sprintf(aux4,"%d",indiceTerceto-1);
				sprintf(aux5,"main_else_%d",valorIndic);
				crearTerceto("ifnot",aux4,aux5);
				
				
				sprintf(aux5,"main_if_%d",valorIndic);
				crearTerceto("label",aux5,"-1");
				
				sprintf(aux5,"%d",valorIndic);
				push(STACK_BIFILTER,aux5);
				
				sprintf(aux4,"%d",filter_terceto.numeroTerceto);
				sprintf(aux5,"%d",var.numeroTerceto);
				crearTerceto(":=",aux4,aux5);

	
				
				sprintf(aux5,"main_endif_%d",valorIndic);
				crearTerceto("jump",aux5,"-1");


				for(cont=0;cont<cantAnd;cont++)
				{
					sprintf(vectorTercetos[pop(STACK_AND)].t2 ,"%d",indiceTerceto);
				}				
	...
	{
			 //******* validacion **************//
				int pos = buscarEnVector($<str_val>3);


				if(pos==-1) // no existe en la tabla
				{
					printf("acaaaaa %s\n",$<str_val>3);
					mostrarError(ERROR_ID_NO_DECLARADO,$<str_val>3);
				}
				else
					if(strcmp(vectorSymbol[pos].tipo,"integer"))
					{
						mostrarError(ERROR_TIPO_INCOMPATIBLE,$<str_val>3);
					}	
					
					int valorIndic=pop(STACK_BIFILTER);
					sprintf(aux5,"main_else_%d",valorIndic);
					sprintf(aux4,"%d",valorIndic);
					push(STACK_BIFILTER,aux4);
					crearTerceto("label",aux5,"-1");
				int op1;
				cantidadVar++;
				// busco en vector de tercetos
					...
					if(contOperLog==0) // cuando no hay operadores
						...
						{	if(cont>0) // si no es la primer pasada
							...
							if(strcmp(vecOpLog[cant], "AND")==0)  // SI ES UN AND T2 DEBE APUNTAR A DONDE TERMINA EL IF, SE DEBE APILAR
								...
								{	// SI ES OR SE COMPLETA T2 DEL ANTERIOR CON EL CMP ACTUAL , SE GUARDA ACTUAL
									//


									//NUEVO, APILO LA PRIMER CONDICION
...
t_terceto crearTercetoFilter(char* operacion,char* t1,char* t2){
	...
	if(indiceTercetoFilter > 0 && aux != NULL)
		...
		cantElemFilter++;
		vectorTercetosFilter = (t_terceto*) realloc(vectorTercetosFilter, sizeof(t_terceto) * (cantElemFilter));
		if(vectorTercetosFilter == NULL)
			mostrarError(ERROR_MEMORIA, "realloc del vectorTercetosFilter en crearTerceto()");
		vectorTercetosFilter[cantElemFilter-1] = result;	
	...
	{ 
		...
		{			
			...
			if (strcmp(tipos[pos].tipo, "integer")==0
				...
				|| strcmp(tipos[pos].tipo, "CONST_BIN")==0
				|| strcmp(tipos[pos].tipo, "CONST_HEXA")==0)
			{
				char operacion[50];
				operacion[0]='\0';
...
void agregarVariable(char* variable, char* tipo) {
	...
	if (strcmp(tipo,"integer")==0 || strcmp(tipo,"CONST_INT")==0 || strcmp(tipo,"CONST_BIN")==0 || strcmp(tipo,"CONST_HEXA")==0 ) {
		strcpy(valor,"?");
	}
